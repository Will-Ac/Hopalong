...
export function createHelpOverlay(options) {
  ...
  function render() {
    ...
    if (isMobile) {
      ...
      const formulaLayoutFinal = layouts.find((item) => item.group.id === "formula-cmap");
      if (formulaLayoutFinal && sliderLayout && doLayoutsOverlap(formulaLayoutFinal, sliderLayout, 2)) {
        sliderLayout.x = clamp(formulaLayoutFinal.x + formulaLayoutFinal.width + 10, minLeftBound, viewportWidth - sliderLayout.width - margin);
      }
    } else {
      preventLabelOverlap(layouts, viewportWidth, viewportHeight, {
        startY: margin,
