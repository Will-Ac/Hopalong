...
const FORMULA_DEFS = [
  { id:"classic_sqrt", name:"Classic (sqrt)", desc:"Wolfram/Martin: x' = y − sgn(x)·sqrt(|bx − c|), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*x-d)), a - x ]; } },

  { id:"sqrt_plus_gamma_y", name:"Classic + dy", desc:"x' = y − sgn(x)·sqrt(|bx − c|) + dy, y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*y, a - x ]; } },

  { id:"sqrt_plus_gamma_x", name:"Classic + dx", desc:"x' = y − sgn(x)·sqrt(|bx − c|) + dx, y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*x, a - x ]; } },

  { id:"mix_inside", name:"sqrt(|b(x+dy)−c|)", desc:"Mix x,y inside sqrt: x' = y − sgn(x)·sqrt(|b(x+dy) − c|), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*(x+g*y)-d)), a - x ]; } },

  { id:"trig_kick_x", name:"Trig kick (sin x)", desc:"x' = y − sgn(x)·sqrt(|bx − c|) + d·sin(x), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*Math.sin(x), a - x ]; } },

  { id:"damped", name:"Damped (|d|)", desc:"x' = (y − sgn(x)·sqrt(|b x − c|))·(1−|d|), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const base=y - s*Math.sqrt(Math.abs(b*x-d)); const damp=1-Math.min(0.95,Math.abs(g)); return [ base*damp, a - x ]; } },

  { id:"y_feedback", name:"Y feedback (a−x+d·y)", desc:"Keep classic x', add feedback on y: y' = a − x + d·y",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const xn=y - s*Math.sqrt(Math.abs(b*x-d)); const yn=a - x + g*y; return [xn, yn]; } },

  { id:"trig_kick_y", name:"Trig kick (sin y)", desc:"x' = y − sgn(x)·sqrt(|b x − c|) + d·sin(y), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*Math.sin(y), a - x ]; } },

  { id:"classic_plus_yy", name:"Classic + d·y²", desc:"x' = y − sgn(x)·sqrt(|b x − c|) + d·y², y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*(y*y), a - x ]; } },

  { id:"cos_xy_kick", name:"Trig kick (cos(x+y))", desc:"x' = y − sgn(x)·sqrt(|bx − c|) + d·cos(x+y), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*Math.cos(x+y), a - x ]; } },

  { id:"inside_sin_y", name:"Inside sqrt sin(y)", desc:"x' = y − sgn(x)·sqrt(|b(x+d·sin(y)) − c|), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const t=x + g*Math.sin(y); return [ y - s*Math.sqrt(Math.abs(b*t-d)), a - x ]; } },

  { id:"inside_cos_x", name:"Inside sqrt cos(x)", desc:"x' = y − sgn(x)·sqrt(|b(x+d·cos(x)) − c|), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const t=x + g*Math.cos(x); return [ y - s*Math.sqrt(Math.abs(b*t-d)), a - x ]; } },

  { id:"softsign_kick", name:"Softsign kick", desc:"x' = y − sgn(x)·sqrt(|bx − c|) + d·(x/(1+|x|)), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const k = x/(1+Math.abs(x)); return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*k, a - x ]; } },

  { id:"tanh_kick", name:"Tanh kick", desc:"x' = y − sgn(x)·sqrt(|bx − c|) + d·tanh(x), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const k = Math.tanh(x); return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*k, a - x ]; } },

  { id:"sign_xy", name:"Sign of (x·y)", desc:"Use sign(x·y): x' = y − sgn(xy)·sqrt(|bx − c|), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const p=x*y; const s=p>0?1:(p<0?-1:0); return [ y - s*Math.sqrt(Math.abs(b*x-d)), a - x ]; } },

  { id:"double_root", name:"Double-root kick", desc:"x' = y − sgn(x)·(sqrt(|bx−c|)+d·sqrt(|by−c|)), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const r1=Math.sqrt(Math.abs(b*x-d)); const r2=Math.sqrt(Math.abs(b*y-d)); return [ y - s*(r1 + g*r2), a - x ]; } },

  { id:"xy_coupling", name:"XY coupling", desc:"x' = y − sgn(x)·sqrt(|bx − c|) + d·(x·y/50), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const k=(x*y)/50; return [ y - s*Math.sqrt(Math.abs(b*x-d)) + g*k, a - x ]; } },
  ...
  { id:"sinusoidal_hopalong", name:"Sinusoidal Hopalong", desc:"x' = y + sin(bx − c), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ return [ y + Math.sin(b*x - d), a - x ]; } },

  { id:"threeply", name:"Threeply (Peters)", desc:"x' = y − sgn(x)·|sin(x)cos(b) + d − x·sin(a+b+d)|, y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const t = Math.sin(x)*Math.cos(b) + g - x*Math.sin(a+b+g); return [ y - s*Math.abs(t), a - x ]; } },

  { id:"quadrup2", name:"Quadrup-2 (Peters)", desc:"x' = y − sgn(x)·sin(ln|bx−c|)·atan((ln|cx−b|)^2), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const ln1=Math.log(Math.abs(b*x-d)+1e-12); const ln2=Math.log(Math.abs(d*x-b)+1e-12); const k=Math.sin(ln1)*Math.atan((ln2*ln2)); return [ y - s*k, a - x ]; } },

  { id:"chip", name:"Chip (Peters)", desc:"x' = y − sgn(x)·cos((ln|bx−c|)^2)·atan((ln|bx−c|)^2), y' = a − x",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const s=x>0?1:(x<0?-1:0); const ln=Math.log(Math.abs(b*x-d)+1e-12); const t=ln*ln; const k=Math.cos(t)*Math.atan(t); return [ y - s*k, a - x ]; } },

  { id:"pickover_Clifford", disabled:false, name:"Pickover/Clifford", desc:"x' = sin(by) + d·sin(bx), y' = sin(ax) + c·sin(ay) (scaled)",
    stepNoAlloc:(x,y,a,b,g,d)=>{ const S=20; return [ S*(Math.sin(b*y) + g*Math.sin(b*x)), S*(Math.sin(a*x) + d*Math.sin(a*y)) ]; } },

  { id:"peter_de_jong", name:"Peter de Jong", desc:"x' = sin(a·y) − cos(b·x), y' = sin(c·x) − cos(d·y)",
    stepNoAlloc:(x,y,a,b,c,d)=>[ Math.sin(a*y)-Math.cos(b*x), Math.sin(c*x)-Math.cos(d*y) ] },

  { id:"clifford", name:"Clifford (Pickover)", desc:"x' = sin(a·y) + c·cos(a·x), y' = sin(b·x) + d·cos(b·y)",
    stepNoAlloc:(x,y,a,b,c,d)=>[ Math.sin(a*y)+c*Math.cos(a*x), Math.sin(b*x)+d*Math.cos(b*y) ] },

  { id:"fractal_dream", name:"Fractal Dream", desc:"x' = sin(y·b) + c·sin(x·b), y' = sin(x·a) + d·sin(y·a)",
    stepNoAlloc:(x,y,a,b,c,d)=>[ Math.sin(y*b) + c*Math.sin(x*b), Math.sin(x*a) + d*Math.sin(y*a) ] },

  { id:"tinkerbell", name:"Tinkerbell map", desc:"x' = x² − y² + a·x + b·y, y' = 2xy + c·x + d·y",
    stepNoAlloc:(x,y,a,b,c,d)=>[ x*x-y*y+a*x+b*y, 2*x*y+c*x+d*y ] },

  { id:"henon", name:"Hénon (with offsets)", desc:"x' = 1 − a·x² + y + c, y' = b·x + d",
    stepNoAlloc:(x,y,a,b,c,d)=>[ 1-a*x*x+y+c, b*x+d ] },

  { id:"lozi", name:"Lozi (with offsets)", desc:"x' = 1 − a·|x| + y + c, y' = b·x + d",
    stepNoAlloc:(x,y,a,b,c,d)=>[ 1-a*Math.abs(x)+y+c, b*x+d ] },

  { id:"ikeda", name:"Ikeda (4-param)", desc:"t = c − d/(1 + x² + y²); x' = a + b(x cos t − y sin t); y' = b(x sin t + y cos t)",
    stepNoAlloc:(x,y,a,b,c,d)=>{ const r2=x*x+y*y; const t=c-d/(1+r2); const ct=Math.cos(t); const st=Math.sin(t); return [a+b*(x*ct-y*st), b*(x*st+y*ct)]; } },

  { id:"gingerbread", name:"Gingerbreadman (generalized)", desc:"x' = a − y + b|x|, y' = c·x + d",
    stepNoAlloc:(x,y,a,b,c,d)=>[ a - y + b*Math.abs(x), c*x + d ] },

  { id:"popcorn", name:"Popcorn", desc:"x' = x − a·sin(y + tan(3y)) + c, y' = y − b·sin(x + tan(3x)) + d",
    stepNoAlloc:(x,y,a,b,c,d)=>{ const safeTan=(v)=>{ const t=Math.tan(v); return Number.isFinite(t)?Math.max(-10,Math.min(10,t)):0; }; return [ x - a*Math.sin(y + safeTan(3*y)) + c, y - b*Math.sin(x + safeTan(3*x)) + d ]; } },

  { id:"bedhead", name:"Bedhead", desc:"x' = sin(xy/b)·y + cos(ax−y) + d, y' = x + sin(y)/c",
    stepNoAlloc:(x,y,a,b,c,d)=>{ const safeB=Math.abs(b)<1e-9?(b<0?-1e-9:1e-9):b; const safeC=Math.abs(c)<1e-9?(c<0?-1e-9:1e-9):c; return [Math.sin((x*y)/safeB)*y + Math.cos(a*x-y) + d, x + Math.sin(y)/safeC]; } },

  { id:"jason_rampe_1", name:"Jason Rampe 1", desc:"x' = cos(y·b) + c·sin(x·b), y' = cos(x·a) + d·sin(y·a)",
    stepNoAlloc:(x,y,a,b,c,d)=>[ Math.cos(y*b) + c*Math.sin(x*b), Math.cos(x*a) + d*Math.sin(y*a) ] },

  { id:"jason_rampe_2", name:"Jason Rampe 2", desc:"x' = cos(y·b) + c·cos(x·b), y' = cos(x·a) + d·cos(y·a)",
    stepNoAlloc:(x,y,a,b,c,d)=>[ Math.cos(y*b) + c*Math.cos(x*b), Math.cos(x*a) + d*Math.cos(y*a) ] },

  { id:"jason_rampe_3", name:"Jason Rampe 3", desc:"x' = sin(y·b) + c·cos(x·b), y' = cos(x·a) + d·sin(y·a)",
    stepNoAlloc:(x,y,a,b,c,d)=>[ Math.sin(y*b) + c*Math.cos(x*b), Math.cos(x*a) + d*Math.sin(y*a) ] },

  { id:"johnny_svensson", name:"Johnny Svensson", desc:"x' = d·sin(x·a) − sin(y·b), y' = c·cos(x·a) + cos(y·b)",
    stepNoAlloc:(x,y,a,b,c,d)=>[ d*Math.sin(x*a) - Math.sin(y*b), c*Math.cos(x*a) + Math.cos(y*b) ] },

  { id:"gumowski_mira", name:"Gumowski–Mira", desc:"f(u)=c·u+2(1−c)u²/(1+u²); x'=y+a(1−by²)y+f(x), y'=-x+d·f(x')",
    stepNoAlloc:(x,y,a,b,c,d)=>{ const f=(u)=>c*u + (2*(1-c)*u*u)/(1+u*u); const x1=y + a*(1-b*y*y)*y + f(x); return [x1, -x + d*f(x1)]; } },
...
export const FORMULA_STEP_NO_ALLOC_BY_ID = new Map(
  FORMULA_DEFS.map((formulaDef) => [formulaDef.id, formulaDef.stepNoAlloc]),
);
